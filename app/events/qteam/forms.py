...
class QteamForm(FlaskForm):
    how_many_centers = StringField(_l('Hoeveel QTeam service centers zijn er in België aanwezig?'), validators=[DataRequired()])
    which_type_car = StringField(_l('Met welk type wagen kunt u bij een QTeam service center terecht?'), validators=[DataRequired()])

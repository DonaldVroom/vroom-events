...
def send_results_ford():
    ford_events = FordBroncoEvent.query.all()

    ford_events_str = create_table(ford_events, FordBroncoEvent)

    html_content = f"<h3>Ford Events</h3>{ford_events_str}"

    # Update the following line to include sender parameter
    subject="VROOM events: Ford"
